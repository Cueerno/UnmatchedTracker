...
public class CardService {
    ...
    public List<CardDto> getAll(String sortBy, String direction) {
        ...
        List<CardDto> cards = deckMapper.toCardDtos(cardRepository.findAll(getSort(sortBy, direction)));
    ...
    @Transactional(readOnly = true)
    public CardDto getByTitle(String title) {
        log.debug("[CardService] -> getByTitle called with title={}", title);

        return cardRepository.findByTitle(title).map(card -> {
            CardDto dto = deckMapper.toCardDto(card);
            log.info("[CardService] -> getByTitle finished successfully: card retrieved with title={}", dto.getTitle());
            return dto;
        }).orElseThrow(() -> {
